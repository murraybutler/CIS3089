...
public class Course implements Serializable, Comparable<Course>
{
  ...
  /**
   * CompareTo override to allow for comparison of prefix and course number
   *
   * @param incourse  Course object as comparison
   * @return int  0 = same, 1 = comarator larger, -1 = comparison larger
   * Strictly speaking, mutli-factor compareTo() methods are seriously frowned upon.  Comparators are preferred.
   */
  @Override
    public int compareTo(Course incourse)
    {
      int i = getPrefix().compareTo(incourse.getPrefix());
      if (i != 0) {
        return i;
      }
      
      return Integer.compare(getNumber(), incourse.getNumber());
    }
